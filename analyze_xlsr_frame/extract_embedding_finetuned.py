"""
Extract and Save SSL Embeddings (Fine-tuned AASIST Model)

This script extracts frame-level embeddings from the fine-tuned AASIST model
...
# Add aasist model path
sys.path.insert(0, '/home/woongjae/ADD_LAB/Representation-grounded_XAI/analyze_xlsr_frame/aasist')
from aasist.model_embedding import Model
...
def extract_and_save_embeddings(
    ...
    """
    Extract SSL embeddings using fine-tuned AASIST model.

    Args:
        attack_dict: Dictionary mapping attack types to file paths
        output_dir: Directory to save embeddings
        model_path: Path to fine-tuned AASIST model
    ...
    # Initialize fine-tuned AASIST model
    print("="*80)
    print("Initializing Fine-tuned AASIST Model")
    print("="*80)
    ...
    # Load model (following main_SSL_DF_embedding.py)
    ...
    # Extract SSL frontend (access via model.module because of DataParallel)
        ...
        for file_path in tqdm(files_to_process, desc=f"{attack_type}"):
            try:
                ...
                if not shape_printed:
                    ...
                    print(f"  Model: Fine-tuned AASIST (SSL frontend only)")
                    shape_printed = True
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract and save SSL embeddings using fine-tuned AASIST model"
    )
    parser.add_argument(
        '--protocol_path',
        type=str,
    ...
    parser.add_argument(
        '--model_path',
        type=str,
        default='/home/woongjae/AudioXplain/SSL_aasist/Best_LA_model_for_DF.pth',
        help='Path to fine-tuned AASIST model'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
        default='/nvme3/wj/embeddings_finetuned/',
        help='Directory to save embeddings (default: /nvme3/wj/embeddings_finetuned/)'
    )
    parser.add_argument(
        '--device',
        type=str,
    ...
    print("="*80)
    print("Fine-tuned AASIST Embedding Extraction")
    print("="*80)
