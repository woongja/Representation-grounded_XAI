"""
Temporal Difference Learning Analysis for ITW Dataset
...
5. Plots results: bonafide vs spoof
"""
...
def compute_statistics_from_embeddings(embedding_dir: str):
    ...
    Returns:
        mean_results: {label: [μ1, μ2, ...]}
        std_results: {label: [σ1, σ2, ...]}
    """
    ...
    print("="*80)
    print("Temporal Difference Learning Analysis (ITW Dataset)")
    print("="*80)
    ...
    for label in labels:
        ...
        print(f"\n[INFO] Processing {label}: {len(pt_files)} files")
        ...
        mean_results[label] = means
        std_results[label] = stds
...
def plot_results(mean_results, std_results, output_dir):
    """
    Create boxplots comparing bonafide vs spoof
    """
    ...
    spoof_mean = mean_results.get("spoof", [])
    spoof_std = std_results.get("spoof", [])

    if len(bonafide_mean) == 0 or len(spoof_mean) == 0:
    ...
    mean_data = [bonafide_mean, spoof_mean]
    std_data  = [bonafide_std, spoof_std]
    ...
    bp1['boxes'][0].set_facecolor('#2ecc71')  # Green for bonafide
    bp1['boxes'][1].set_facecolor('#e74c3c')  # Red for spoof
    ...
    # Force Bonafide TOP, Spoof BOTTOM
    ax1.invert_yaxis()
    ...
    bp2['boxes'][0].set_facecolor('#2ecc71')  # Green for bonafide
    bp2['boxes'][1].set_facecolor('#e74c3c')  # Red for spoof
    ...
    # Save
    save_path = os.path.join(output_dir, "bonafide_vs_spoof_itw.png")
...
def print_statistics(mean_results, std_results):
    ...
    """
    print("\n" + "="*80)
    print("STATISTICS (ITW Dataset)")
    print("="*80)
    print(f"{'Label':<10} {'μ_mean':>10} {'μ_std':>10} {'σ_mean':>10} {'σ_std':>10} {'Count':>8}")
    print("-"*80)

    labels = ["bonafide", "spoof"]

    for label in labels:
        if label not in mean_results:
            continue

        μ_vals = np.array(mean_results[label])
        σ_vals = np.array(std_results[label])
...
def main():
    parser = argparse.ArgumentParser(
        description="Temporal Difference Learning Analysis for ITW Dataset"
    )
    parser.add_argument(
        '--embedding_dir',
        type=str,
    ...
    # Save results
    result_path = os.path.join(args.output_dir, "results_itw.pkl")
