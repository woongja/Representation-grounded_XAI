"""
t-SNE Visualization of SSL Embeddings for ITW Dataset (Frame-level)

This script:
1. Loads saved embeddings (T, 1024) for bonafide and spoof
2. Samples frames from embeddings
3. Applies t-SNE dimensionality reduction on frame-level embeddings
...
# =============================================================================
# Load Embeddings (Frame-level)
# =============================================================================
def load_embeddings_frame_level(embedding_dir: str, max_files_per_label: int = 100,
                                max_frames_per_file: int = 50):
    """
    Load embeddings from saved .pt files and sample frames.
    ...
    Returns:
        embeddings: numpy array of shape (N, 1024) where N is number of frames
        labels: list of labels (one per frame)
    """
    ...
    print("="*80)
    print("Loading Embeddings for t-SNE (Frame-level) - ITW Dataset")
    print("="*80)
    ...
    for label in labels_to_load:
        ...
        for pt_file in tqdm(pt_files, desc=f"{label:>10}"):
            try:
                ...
                label_embeddings.append(emb.numpy())
        ...
        if len(label_embeddings) > 0:
            # Concatenate all frames from this label
            label_embeddings = np.vstack(label_embeddings)  # (N, 1024)
            all_embeddings.append(label_embeddings)
            all_labels.extend([label] * len(label_embeddings))

            print(f"[INFO] {label}: {len(label_embeddings)} frames loaded")
...
def plot_tsne(embeddings, labels, output_path, perplexity=30, n_iter=1000,
    ...
    ax.set_title("t-SNE Visualization: Bonafide vs Spoof (ITW - Frame-level)", fontsize=16, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description="t-SNE Visualization of SSL Embeddings for ITW Dataset (Frame-level)"
    )
    parser.add_argument(
        '--embedding_dir',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help='Maximum number of files to load per label (default: 100)'
    )
    parser.add_argument(
        '--max_frames_per_file',
        type=int,
    ...
    print("="*80)
    print("t-SNE Visualization (Frame-level) - ITW Dataset")
    print("="*80)
    print(f"Embedding dir: {args.embedding_dir}")
    print(f"Output dir: {args.output_dir}")
    print(f"Max files per label: {args.max_files_per_attack}")
    print(f"Max frames per file: {args.max_frames_per_file}")
    print("="*80)

    # Load embeddings (frame-level)
    embeddings, labels = load_embeddings_frame_level(
        embedding_dir=args.embedding_dir,
        max_files_per_label=args.max_files_per_attack,
        max_frames_per_file=args.max_frames_per_file
    )

    # Plot t-SNE
    output_path = os.path.join(args.output_dir, "tsne_itw_frame.png")
    plot_tsne(
