...
def eval_model(args, config, device):
    ...
    # Dataset - pass args for fusion models
    if config["data"]["name"] == "data_utils_fusion":
        eval_set = Dataset_eval(list_IDs=file_eval, base_dir=args.database_path, args=args)
    else:
    ...
    with torch.no_grad(), open(args.eval_output, 'w') as fh:
        for batch_data in tqdm(eval_loader, desc="Evaluation", leave=False):
            # Handle both formats: (batch_x, utt_id) and (batch_x, batch_mel, utt_id)
            if len(batch_data) == 2:
                batch_x, utt_id = batch_data
                batch_x = batch_x.to(device)
                logits, _ = model(batch_x)
            else:  # len == 3, fusion dataset
                batch_x, batch_mel, utt_id = batch_data
                batch_x = batch_x.to(device)
...
def train_epoch(train_loader, model, optimizer, device, epoch, use_balance_training=True, use_curriculum=False, curriculum_stage=1):
    ...
    if use_balance_training or use_curriculum:
        ...
        for batch_data in pbar:
            # Handle both formats: (x, y) and (x, mel_spec, y)
            if len(batch_data) == 2:
...
def eval_epoch(dev_loader, model, device, epoch, use_balance_training=True):
    ...
    with torch.no_grad():
        if use_balance_training:
            ...
            # BASELINE TRAINING MODE - dev loader returns (batch_x, batch_y) or (batch_x, batch_mel, batch_y)
            for batch_data in pbar:
                # Handle both formats
                if len(batch_data) == 2:
                    batch_x, batch_y = batch_data
...
def main(args):
    ...
    # Check if config explicitly specifies balance training
    use_balance_training = config["data"].get("use_balance_training", False) or \
                          (hasattr(data_module, 'BalancedNoiseSampler') or
                           hasattr(data_module, 'CategoryBalancedSampler')) and not use_curriculum
    ...
    if use_curriculum:
        ...
        # Check if this is hard_routing (uses DataLoader) or balance (uses custom sampler)
        use_dataloader_mode = config["data"]["name"] == "data_utils_hard_routing"
        ...
        if use_dataloader_mode:
            # Hard routing mode: Use DataLoader with CategoryBalancedSampler
            print("[INFO] Using DataLoader mode with CategoryBalancedSampler (for hard_routing)")

            # Create training dataset
            train_set = Dataset_for(
                list_IDs=file_train,
                labels=d_label_trn,
                noise_dict=noise_trn,
                base_dir=args.database_path,
                algo=args.algo
            )
            ...
            # Balance mode: Use custom sampler that generates tuples
            print("[INFO] Using custom sampler mode (for data_utils_balance)")
